...
const AdminChangeDialog = ({open, closeWindow} : {open : boolean , closeWindow : ()=>void } ) => {
    ...
    const fetchUsers = () => {
        axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/channel/users/without?spaceId=${space}&channelId=${channel}`, { withCredentials: true })
            .then((res) => {
    ...
    const changeAdmin = () => {
        if (selectedUser) {
            axios.post(
                `${process.env.NEXT_PUBLIC_API_URL}/api/v1/channel/yield`,
                {spaceId: space,channelId:channel,userId: selectedUser.id}, // 사용자 입력값 사용
                {
